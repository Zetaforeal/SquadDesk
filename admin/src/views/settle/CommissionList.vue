<template>
  <el-card shadow="never">
    <el-table :data="rows" v-loading="loading" stripe>
      <el-table-column prop="order?.orderNo" label="订单号" width="200" />
      <el-table-column prop="player?.nickname" label="打手" width="120">
        <template #default="{ row }">{{ row.player?.nickname || '-' }}</template>
      </el-table-column>
